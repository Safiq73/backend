...
class Settings(BaseSettings):
    ...
    # Recommendation System (Phase 0 flags)
    enable_recommendations: bool = True
    recs_exploration_rate: float = 0.05  # 5% of slots reserved for exploration
    recs_cache_ttl_seconds: int = 60

    # Enable/disable candidate sources
    recs_enable_social: bool = True
    recs_enable_trending: bool = True
    recs_enable_locality: bool = True
    recs_enable_recency: bool = True

    # Heuristic ranker weights
    recs_weight_personal_affinity: float = 0.35
    recs_weight_engagement_rate: float = 0.20
    recs_weight_recency_decay: float = 0.20
    recs_weight_social_proximity: float = 0.15
    recs_weight_locality_match: float = 0.10

    # Diversity and safety caps
    recs_diversity_max_per_author: int = 3
    recs_diversity_max_per_topic: int = 5
    recs_overexposure_impressions_24h: int = 3

    # Event sampling
    recs_sample_impressions_rate: float = 0.33  # ~1 in 3 impressions logged

    # Recommendation Scheduler (Phase 2)
    recs_enable_scheduler: bool = True
    recs_job_quality_interval_seconds: int = 600      # 10 minutes
    recs_job_affinity_interval_seconds: int = 1800    # 30 minutes
    recs_job_cleanup_interval_seconds: int = 86400    # 24 hours
    recs_cleanup_retention_days: int = 180            # keep 6 months of raw interactions
