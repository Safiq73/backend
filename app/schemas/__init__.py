...
class UserBase(BaseModel):
    ...
    base_latitude: Optional[float] = Field(None, description="Base latitude coordinate (India bounds: 6.5° to 37.5° N)")
    base_longitude: Optional[float] = Field(None, description="Base longitude coordinate (India bounds: 68° to 97.5° E)")
    ...
    @validator('base_latitude')
    def validate_base_latitude(cls, v):
        """Validate base latitude is within India bounds"""
        if v is not None:
            # India bounds: approximately 6.5° to 37.5° N
            if not (6.5 <= v <= 37.5):
                raise ValueError('Base latitude must be within India boundaries (6.5° to 37.5° N)')
        return v

    @validator('base_longitude')
    def validate_base_longitude(cls, v):
        """Validate base longitude is within India bounds"""
        if v is not None:
            # India bounds: approximately 68° to 97.5° E
            if not (68.0 <= v <= 97.5):
                raise ValueError('Base longitude must be within India boundaries (68° to 97.5° E)')
        return v
...
class UserUpdate(BaseModel):
    ...
    # Base location fields for profile updates
    base_latitude: Optional[float] = Field(None, ge=6.5, le=37.5, description="Base location latitude (India bounds)")
    base_longitude: Optional[float] = Field(None, ge=68.0, le=97.5, description="Base location longitude (India bounds)")
...
class PostResponse(PostBase):
    ...
    is_following: Optional[bool] = None  # Whether current user follows the post author
    is_followed_by: Optional[bool] = None  # Whether post author follows current user
    follow_mutual: Optional[bool] = None  # Whether they follow each other
